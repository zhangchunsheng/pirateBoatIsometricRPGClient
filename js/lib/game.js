/**
 * (<b>w</b>)ozllaJS (<b>e</b>)ngine : All wozllaJS functions are defined inside of this namespace.<p>
...
(function($, undefined) {
	...
	me.sys = {
		 ...
		 * if me.debug.stopOnAudioLoad is true, wozllaJS will throw an exception and stop loading<br>
		 * if me.debug.stopOnAudioLoad is false, wozllaJS will disable sounds and output a warning message in the console <br>
	...
	if (!Object.defineProperty) {
		...
		Object.defineProperty = function(obj, prop, desc) {
			...
			if (obj.__defineGetter__) {
				...
				throw "wozllaJS: Object.defineProperty not supported";
			}
		}
	};
	...
	me.state = (function() {
		...
		if (!window.requestAnimationFrame) {
			window.requestAnimationFrame = function(callback, element) {
				...
				// in wozllaJS if this returns -1 clearInterval is used
				return -1;
			};
        };
...
(function($, undefined) {
	...
	me.loader = (function() {
		...
		obj.onLoadingError = function(res) {
			throw "wozllaJS: Failed loading resource " + res.src;
		};
		...
		obj.load = function(res, onload, onerror) {
			...
			switch (res.type) {
				...
				default:
					throw "wozllaJS: me.loader.load : unknow or invalide resource type : %s"	+ res.type;
					break;
			};
			return 0;
		};
			...
			/** @scope me.Viewport.prototype */
			{
				...
				follow : function(target, axis) {
					if (target instanceof me.ObjectEntity)
						...
						throw "wozllaJS: invalid target for viewport.follow";
	...
	me.audio = (function() {
		...
		function soundLoadError(sound_id) {
			...
			if (retry_counter++ > 3) {
				...
				var errmsg = "wozllaJS: failed loading " + sound_id + "." + activeAudioExt;
		 ...
		 * the wozllaJS loader will try to load audio files corresponding to the
	...
	me.input = (function() {
		...
		obj.bindMouse = function (button, keyCode)
		{
			...
			if (!KeyBinding[keyCode])
			  throw "wozllaJS : no action defined for keycode " + keyCode;
		...
		obj.registerMouseEvent = function(eventType, rect, callback, floating) {
			...
			switch (eventType) {
				...
				case 'touchend':
					...
					break;
				default :
					throw "wozllaJS : invalid event type : " + eventType;
			}
		};
...
(function($, undefined) {
	...
	me.Tileset = Object.extend({
		...
		init: function (name, tilewidth, tileheight, spacing, margin, imagesrc) {
			...
			if (!this.image) {
				console.log("wozllaJS: '" + imagesrc + "' file for tileset '" + this.name + "' not found!");
			}
				...
				for (var x = startPos.x; x < rectEnd.x; x += this.tilewidth) {
					...
					if ((columnItr.x >= 0) && (columnItr.y >= 0) && (columnItr.x < this.width) && (columnItr.y < this.height))
					{
						...
						if (tmxTile) {
							...
							if(btg.debug) {
								if(tileset.name == "tiled_collision") {
									context.strokeStyle = "green";
									//context.strokeRect(x, y - this.tileheight, tileset.tilewidth, tileset.tileheight);
								}
							}
						}
					}
	 ...
	 */
	 me.ImageLayer = Object.extend({
		...
		init: function(name, width, height, imagesrc, z, ratio) {
			...
			this.image = (imagesrc) ? me.loader.getImage(me.utils.getBasename(imagesrc)) : null;
			if (!this.image) {
				console.log("wozllaJS: '" + imagesrc + "' file for Image Layer '" + this.name + "' not found!");
			}
		...
		init: function(layer, tilewidth, tileheight, orientation, tilesets, zOrder) {
			...
			if (this.visible) {
				...
				switch (this.orientation)
				{
					...
					default : {
						throw "wozllaJS: " + this.orientation + " type TMX Tile Map not supported!";
					}
				}
			...
			switch (compression) {
			 ...
			 case null: {
				...
				// XML encoding
				   ...
				   default:
					  throw "wozllaJS: TMX Tile Map " + encoding + " encoding not supported!";
					  break;
				}
			 ...
			 default:
				throw "wozllaJS: " + compression+ " compressed TMX Tile Map not supported!";
				break;
			}
...
(function($, undefined) {
	...
	me.TMXTileMap = me.TileMap.extend({
		...
		init: function(xmlfile, x, y) {
			...
			if (!this.xmlMap) {
				throw "wozllaJS:" + xmlfile + " TMX map not found";
			};
...
(function($, undefined) {
	...
	me.levelDirector = (function() {
		...
		obj.addLevel = function(level) {
			throw "wozllaJS: no level loader defined";
		};
		...
		obj.loadLevel = function(levelId) {
			...
			if (levels[levelId] === undefined) {
				throw ("wozllaJS: level " + levelId + " not found");
			}
